 ...
 * Copyright 2017 Google LLC
 * SPDX-License-Identifier: BSD-3-Clause
 */function ft(t){return pt({...t,state:!0,attribute:!1})}const _t="pulseline-card";function gt(t){const e=[];for(const s of t){const t=s.s??s.state;if(!t)continue;const i=parseFloat(t);if(isNaN(i))continue;let n;if(null!=s.lu)n=1e3*s.lu;else{if(!s.last_updated)continue;n=new Date(s.last_updated).getTime()}e.push({value:i,timestamp:n})}return e}function vt(t){if(0===t.length)return[];const e=[t[0]];let s=Math.round(100*t[0]);for(let i=1;i<t.length;i++){const n=Math.round(100*t[i]);n!==s&&(e.push(t[i]),s=n)}return e}async function yt(t,e,s){const i=new Date,n=new Date(i);n.setDate(n.getDate()-(s-1)),n.setHours(0,0,0,0);try{const i=(await t.callWS({type:"recorder/statistics_during_period",statistic_ids:[e],period:"day",start_time:n.toISOString(),types:["mean"]}))[e];if(i&&i.length>0){const t=new Array(s).fill(null);for(const e of i){if(null==e.mean)continue;const i=new Date(e.start).getTime()-n.getTime(),r=Math.floor(i/864e5);r>=0&&r<s&&(t[r]=e.mean)}const e=t.filter(t=>null!==t).length;if(e>=2)return t}}catch{}return async function(t,e,s,i){try{const n=gt((await t.callWS({type:"history/history_during_period",start_time:i.toISOString(),entity_ids:[e],minimal_response:!0,no_attributes:!0,significant_changes_only:!1}))[e]||[]),r=new Array(s).fill(null),o=new Array(s).fill(0),a=new Array(s).fill(0);for(const t of n){const e=t.timestamp-i.getTime(),n=Math.floor(e/864e5);n>=0&&n<s&&(o[n]+=t.value,a[n]++)}for(let t=0;t<s;t++)a[t]>0&&(r[t]=o[t]/a[t]);return r}catch{return new Array(s).fill(null)}}(t,e,s,n)}const mt=[30,90,365];async function $t(t,e,s){const i=new Date;i.setDate(i.getDate()-s);return gt((await t.callWS({type:"history/history_during_period",start_time:i.toISOString(),entity_ids:[e],minimal_response:!0,no_attributes:!0,significant_changes_only:!1}))[e]||[]).map(t=>t.value)}async function wt(t,e,s){let i=[];for(const n of mt)try{const r=vt(await $t(t,e,n));if(r.length>i.length&&(i=r),i.length>=s)return i.slice(-s)}catch{continue}try{const s=await async function(t,e,s){const i=new Date;return i.setDate(i.getDate()-s),((await t.callWS({type:"recorder/statistics_during_period",statistic_ids:[e],period:"day",start_time:i.toISOString(),types:["mean"]}))[e]||[]).filter(t=>null!=t.mean).map(t=>t.mean)}(t,e,365),n=vt(s);n.length>i.length&&(i=n)}catch{}return i.slice(-s)}console.info(`%c  ${_t.toUpperCase()}  %c  v0.3.0-dev.1  `,"color: white; background: #3b82f6; font-weight: bold; padding: 2px 6px; border-radius: 4px 0 0 4px;","color: #3b82f6; background: #e0e7ff; font-weight: bold; padding: 2px 6px; border-radius: 0 4px 4px 0;");const bt=["Sun","Mon","Tue","Wed","Thu","Fri","Sat"];let xt=class extends ct{constructor(){super(...arguments),this._dailyBuckets=[],this._recentValues=[],this._lastFetchKey="",this._lastFetchTime=0,this._fetchInProgress=!1}static getStubConfig(){return{entity:"sensor.pulseline_preview"}}setConfig(t){if(!t||"object"!=typeof t)throw new Error("Invalid configuration: config must be an object");if(!t.entity||"string"!=typeof t.entity)throw new Error("Invalid configuration: 'entity' is required");const e=t.card_mode||"single";if("single"!==e&&"dual"!==e)throw new Error("Invalid configuration: 'card_mode' must be 'single' or 'dual'");if("dual"===e){if(!t.entity_2||"string"!=typeof t.entity_2)throw new Error("Invalid configuration: 'entity_2' is required when card_mode is 'dual'");if("score"===t.display_style)throw new Error("Invalid configuration: display_style 'score' is not supported in dual mode");if(t.footer_row&&"none"!==t.footer_row.type)throw new Error("Invalid configuration: footer_row is not supported in dual mode");if(t.supporting_row&&"delta"===t.supporting_row.type)throw new Error("Invalid configuration: supporting_row 'delta' is not supported in dual mode")}if(t.display_style&&"unit"!==t.display_style&&"score"!==t.display_style)throw new Error("Invalid configuration: 'display_style' must be 'unit' or 'score'");if("score"===t.display_style&&(null==t.score_max||"number"!=typeof t.score_max))throw new Error("Invalid configuration: 'score_max' is required when display_style is 'score'");if(null!=t.value_precision&&("number"!=typeof t.value_precision||t.value_precision<0))throw new Error("Invalid configuration: 'value_precision' must be a non-negative number");if(t.supporting_row){const e=t.supporting_row.type;if("none"!==e&&"kudos"!==e&&"delta"!==e)throw new Error("Invalid configuration: supporting_row.type must be 'none', 'kudos', or 'delta'");if("kudos"===e){if(!Array.isArray(t.supporting_row.kudos_rules)||0===t.supporting_row.kudos_rules.length)throw new Error("Invalid configuration: 'kudos_rules' must be a non-empty array when type is 'kudos'");for(const e of t.supporting_row.kudos_rules){if("number"!=typeof e.min||"string"!=typeof e.label)throw new Error("Invalid configuration: each kudos rule must have 'min' (number) and 'label' (string)");if(null!=e.max&&"number"!=typeof e.max)throw new Error("Invalid configuration: kudos rule 'max' must be a number if provided");if(null!=e.icon&&"string"!=typeof e.icon)throw new Error("Invalid configuration: kudos rule 'icon' must be a string if provided");if(null!=e.color&&"string"!=typeof e.color)throw new Error("Invalid configuration: kudos rule 'color' must be a string if provided");if(null!=e.accent_override&&"boolean"!=typeof e.accent_override)throw new Error("Invalid configuration: kudos rule 'accent_override' must be a boolean if provided")}}}if(t.footer_row){const e=t.footer_row.type;if("none"!==e&&"sparkline_days"!==e&&"sparkline_values"!==e&&"progress_bar"!==e)throw new Error("Invalid configuration: footer_row.type must be 'none', 'sparkline_days', 'sparkline_values', or 'progress_bar'");if("sparkline_values"===e&&null!=t.footer_row.x_values&&("number"!=typeof t.footer_row.x_values||t.footer_row.x_values<2||t.footer_row.x_values>14))throw new Error("Invalid configuration: x_values must be a number between 2 and 14");if("progress_bar"===e&&"score"!==t.display_style)throw new Error("Invalid configuration: progress_bar footer requires display_style 'score'")}this._config=t}_computeRowSize(){const t=this._config?.footer_row?.type??"none";return"sparkline_days"===t||"sparkline_values"===t?3:2}getCardSize(){return this._computeRowSize()}getGridOptions(){return{columns:6,rows:this._computeRowSize(),min_columns:3,min_rows:1}}updated(t){super.updated(t),(t.has("hass")||t.has("_config"))&&this._scheduleDataFetch()}_needsRecentValues(){const t=this._config?.footer_row?.type;if("sparkline_values"===t)return!0;const e=this._config?.supporting_row?.type;return"delta"===e&&"sparkline_days"!==t}_needsDailyBuckets(){return"sparkline_days"===this._config?.footer_row?.type}_scheduleDataFetch(){if(!this._config||!this.hass)return;const t=this._needsRecentValues(),e=this._needsDailyBuckets();if(!t&&!e)return;const s=t?this._config.footer_row?.x_values||7:0,i=`${this._config.entity}:r${t?s:0}:d${e?7:0}`,n=Date.now();(i!==this._lastFetchKey||n-this._lastFetchTime>3e5)&&(this._fetchInProgress||(this._lastFetchKey=i,this._lastFetchTime=n,this._fetchData()))}async _fetchData(){if(this.hass&&this._config){this._fetchInProgress=!0;try{this._needsDailyBuckets()&&(this._dailyBuckets=await yt(this.hass,this._config.entity,7)),this._needsRecentValues()&&(this._recentValues=await wt(this.hass,this._config.entity,this._config.footer_row?.x_values||7))}finally{this._fetchInProgress=!1}}}_handleClick(){if(!this._config)return;const t=new CustomEvent("hass-more-info",{bubbles:!0,composed:!0,detail:{entityId:this._config.entity}});this.dispatchEvent(t)}_isDualMode(){return"dual"===(this._config?.card_mode||"single")}_getEntity(){if(this.hass&&this._config)return this.hass.states[this._config.entity]}_getEntity2(){if(this.hass&&this._config?.entity_2)return this.hass.states[this._config.entity_2]}_getIcon(t){return this._config.icon?this._config.icon:t?.attributes.icon?t.attributes.icon:"mdi:chart-line"}_getAccentColor(){const t=this._getKudosAccentOverride();return t||(this._config.accent_color||"#3b82f6")}_getKudosAccentOverride(){const t=this._config?.supporting_row;if(!t||"kudos"!==t.type||!t.kudos_rules)return null;const e=this._getEntity();if(!e||this._isEntityUnavailable(e))return null;const s=this._normalizeNumeric(e.state);if(isNaN(s))return null;const i=this._evaluateKudos(s,t.kudos_rules);return i&&i.color&&i.accent_override?i.color:null}_getTitle(t){return this._config.name?this._config.name:t?.attributes.friendly_name?t.attributes.friendly_name:this._config.entity}_isEntityUnavailable(t){const e=t.state;return"unavailable"===e||"unknown"===e}_isValueUnavailable(){const t=this._getEntity();if(!t||this._isEntityUnavailable(t))return!0;if(this._isDualMode()){const t=this._getEntity2();if(!t||this._isEntityUnavailable(t))return!0}return!1}_evaluateKudos(t,e){for(const s of e)if(t>=s.min&&(null==s.max||t<=s.max))return s;return null}_normalizeNumeric(t){const e=this._config.value_precision??0,s=parseFloat(t);return isNaN(s)?NaN:parseFloat(s.toFixed(e))}_isNumericState(t){const e=t.trim();return""!==e&&!isNaN(Number(e))}_formatValue(t){const e=this._normalizeNumeric(t);return isNaN(e)?this._formatNonNumeric(t):e.toFixed(this._config.value_precision??0)}_formatNonNumeric(t){const e=t.replace(/_/g," ").trim();return""===e?"–":e.replace(/\S+/g,t=>t===t.toUpperCase()?t:t.charAt(0).toUpperCase()+t.slice(1))}_renderValueRow(t){if(this._isDualMode())return this._renderDualValueRow(t);if(this._isEntityUnavailable(t))return L`<div class="value-row"><span class="value-suffix">–</span></div>`;const e=this._formatValue(t.state),s=this._isNumericState(t.state);if("score"===(this._config.display_style||"unit")&&s)return L`
        <div class="value-row">
    ...
    `}_renderSupportingRow(t){if(this._isValueUnavailable())return K;const e=this._config.supporting_row;if(!e||"none"===e.type)return K;if("kudos"===e.type&&e.kudos_rules){const s=this._normalizeNumeric(t.state);if(isNaN(s))return K;const i=this._evaluateKudos(s,e.kudos_rules);return i?this._renderKudos(i):K}return"delta"===e.type?this._renderDelta(t):K}_renderKudos(t){const e=!!t.icon,s=t.color||this._getAccentColor();return e&&t.color?L`<div class="supporting-row kudos" style="color: ${t.color};">
        <div class="kudos-badge" style="background: ${s}33; color: ${s};">
    ...
    `}_shouldShowPreview(){return!this._config||!("sensor.pulseline_preview"!==this._config.entity||!this.hass||this.hass.states["sensor.pulseline_preview"])}_renderPreview(){const t="#3B82F6";return L`
      <ha-card>
        <div class="content">
          <div
            class="icon-badge"
            style="background: ${t}33; color: ${t};"
          >
          ...
          <div class="text-block">
            ...
            <div class="supporting-row kudos" style="color: ${t};">
              <div class="kudos-badge" style="background: ${t}33; color: ${t};">
        ...
        ${this._renderSparkline([6100,7200,6900,7600,8100,7900,8243],t,!1)}
      </ha-card>
    `}render(){if(this._shouldShowPreview())return this._renderPreview();if(!this._config||!this.hass)return L``;const t=this._getEntity();if(!t)return L`
        <ha-card>
          <div class="content">
    ...
    `}};t([pt({attribute:!1})],xt.prototype,"hass",void 0),t([ft()],xt.prototype,"_config",void 0),t([ft()],xt.prototype,"_dailyBuckets",void 0),t([ft()],xt.prototype,"_recentValues",void 0),xt=t([(t=>(e,s)=>{void 0!==s?s.addInitializer(()=>{customElements.define(t,e)}):customElements.define(t,e)})(_t)],xt);const At=window;At.customCards=At.customCards||[],At.customCards.push({type:_t,name:"PulseLine Card",description:"A compact metric and trend card with deltas, sparklines, and contextual insights.",preview:!0,documentationURL:"https://github.com/mtwhitley/pulseline-card"});export{xt as PulseLineCard};
